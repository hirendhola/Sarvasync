import React from 'react';

interface IconProps extends React.SVGProps<SVGSVGElement> {
  className?: string;
}

export const HomeIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <defs>
      <linearGradient id="homeGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="currentColor" stopOpacity="1" />
        <stop offset="100%" stopColor="currentColor" stopOpacity="0.6" />
      </linearGradient>
    </defs>
    <path 
      d="M3 10.9C3 9.70243 3.84285 8.67 5.00191 8.46L11.0019 7.26C11.6363 7.14 12.3637 7.14 12.9981 7.26L18.9981 8.46C20.1571 8.67 21 9.70243 21 10.9V19C21 20.1046 20.1046 21 19 21H5C3.89543 21 3 20.1046 3 19V10.9Z" 
      fill="url(#homeGradient)" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fillOpacity="0.1"
    />
    <path 
      d="M9 21V16C9 15.4477 9.44772 15 10 15H14C14.5523 15 15 15.4477 15 16V21" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fill="none"
    />
    <circle cx="12" cy="4" r="2" fill="currentColor" opacity="0.8" />
  </svg>
);

export const ContentIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <defs>
      <linearGradient id="contentGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="currentColor" stopOpacity="1" />
        <stop offset="100%" stopColor="currentColor" stopOpacity="0.6" />
      </linearGradient>
    </defs>
    <path 
      d="M14 2H6C4.89543 2 4 2.89543 4 4V20C4 21.1046 4.89543 22 6 22H18C19.1046 22 20 21.1046 20 20V8L14 2Z" 
      fill="url(#contentGradient)" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fillOpacity="0.1"
    />
    <path d="M14 2V8H20" stroke="currentColor" strokeWidth="1.5" />
    <circle cx="8" cy="13" r="1" fill="currentColor" />
    <circle cx="12" cy="13" r="1" fill="currentColor" />
    <circle cx="16" cy="13" r="1" fill="currentColor" />
    <path d="M8 17H16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
  </svg>
);

export const ScheduleIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <defs>
      <linearGradient id="scheduleGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="currentColor" stopOpacity="1" />
        <stop offset="100%" stopColor="currentColor" stopOpacity="0.6" />
      </linearGradient>
    </defs>
    <rect 
      x="3" y="4" width="18" height="18" rx="3" 
      fill="url(#scheduleGradient)" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fillOpacity="0.1"
    />
    <circle cx="8" cy="2" r="1" fill="currentColor" />
    <circle cx="16" cy="2" r="1" fill="currentColor" />
    <path d="M3 10H21" stroke="currentColor" strokeWidth="1.5" />
    <circle cx="8" cy="14" r="2" fill="currentColor" opacity="0.8" />
    <circle cx="16" cy="18" r="1" fill="currentColor" opacity="0.6" />
  </svg>
);

export const AnalyticsIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <defs>
      <linearGradient id="analyticsGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="currentColor" stopOpacity="1" />
        <stop offset="100%" stopColor="currentColor" stopOpacity="0.6" />
      </linearGradient>
    </defs>
    <path d="M3 3V21H21" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    <path 
      d="M7 16L12 11L16 15L21 10" 
      stroke="url(#analyticsGradient)" 
      strokeWidth="2.5" 
      strokeLinecap="round" 
    />
    <circle cx="7" cy="16" r="2" fill="currentColor" opacity="0.8" />
    <circle cx="12" cy="11" r="2" fill="currentColor" opacity="0.9" />
    <circle cx="16" cy="15" r="2" fill="currentColor" opacity="0.7" />
    <circle cx="21" cy="10" r="2" fill="currentColor" />
  </svg>
);

export const AccountsIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <defs>
      <linearGradient id="accountsGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="currentColor" stopOpacity="1" />
        <stop offset="100%" stopColor="currentColor" stopOpacity="0.6" />
      </linearGradient>
    </defs>
    <circle 
      cx="12" cy="7" r="4" 
      fill="url(#accountsGradient)" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fillOpacity="0.2"
    />
    <path 
      d="M4 19C4 16.7909 5.79086 15 8 15H16C18.2091 15 20 16.7909 20 19V21" 
      fill="url(#accountsGradient)" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fillOpacity="0.1"
    />
    <circle cx="18" cy="6" r="2" fill="currentColor" opacity="0.6" />
    <circle cx="6" cy="6" r="2" fill="currentColor" opacity="0.6" />
  </svg>
);

export const SettingsIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <defs>
      <linearGradient id="settingsGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="currentColor" stopOpacity="1" />
        <stop offset="100%" stopColor="currentColor" stopOpacity="0.6" />
      </linearGradient>
    </defs>
    <circle 
      cx="12" cy="12" r="3" 
      fill="url(#settingsGradient)" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fillOpacity="0.2"
    />
    <path 
      d="M19.4 15C19.2669 15.3016 19.2272 15.6362 19.286 15.9606C19.3448 16.285 19.4995 16.5843 19.73 16.82L19.79 16.88C19.976 17.0657 20.1235 17.2863 20.2241 17.5291C20.3248 17.7719 20.3766 18.0322 20.3766 18.295C20.3766 18.5578 20.3248 18.8181 20.2241 19.0609C20.1235 19.3037 19.976 19.5243 19.79 19.71C19.6043 19.896 19.3837 20.0435 19.1409 20.1441C18.8981 20.2448 18.6378 20.2966 18.375 20.2966C18.1122 20.2966 17.8519 20.2448 17.6091 20.1441C17.3663 20.0435 17.1457 19.896 16.96 19.71L16.9 19.65C16.6643 19.4195 16.365 19.2648 16.0406 19.206C15.7162 19.1472 15.3816 19.1869 15.08 19.32C14.7842 19.4468 14.532 19.6572 14.3543 19.9255C14.1766 20.1938 14.0813 20.5082 14.08 20.83V21C14.08 21.5304 13.8693 22.0391 13.4942 22.4142C13.1191 22.7893 12.6104 23 12.08 23C11.5496 23 11.0409 22.7893 10.6658 22.4142C10.2907 22.0391 10.08 21.5304 10.08 21V20.91C10.0723 20.579 9.96512 20.2579 9.77251 19.9887C9.5799 19.7194 9.31074 19.5143 9 19.4C8.69838 19.2669 8.36381 19.2272 8.03941 19.286C7.71502 19.3448 7.41568 19.4995 7.18 19.73L7.12 19.79C6.93425 19.976 6.71368 20.1235 6.47088 20.2241C6.22808 20.3248 5.96783 20.3766 5.705 20.3766C5.44217 20.3766 5.18192 20.3248 4.93912 20.2241C4.69632 20.1235 4.47575 19.976 4.29 19.79C4.10405 19.6043 3.95653 19.3837 3.85588 19.1409C3.75523 18.8981 3.70343 18.6378 3.70343 18.375C3.70343 18.1122 3.75523 17.8519 3.85588 17.6091C3.95653 17.3663 4.10405 17.1457 4.29 16.96L4.35 16.9C4.58054 16.6643 4.73519 16.365 4.794 16.0406C4.85282 15.7162 4.81312 15.3816 4.68 15.08C4.55324 14.7842 4.34276 14.532 4.07447 14.3543C3.80618 14.1766 3.49179 14.0813 3.17 14.08H3C2.46957 14.08 1.96086 13.8693 1.58579 13.4942C1.21071 13.1191 1 12.6104 1 12.08C1 11.5496 1.21071 11.0409 1.58579 10.6658C1.96086 10.2907 2.46957 10.08 3 10.08H3.09C3.42099 10.0723 3.742 9.96512 4.01127 9.77251C4.28053 9.5799 4.48572 9.31074 4.6 9C4.73312 8.69838 4.77282 8.36381 4.714 8.03941C4.65519 7.71502 4.50054 7.41568 4.27 7.18L4.21 7.12C4.02405 6.93425 3.87653 6.71368 3.77588 6.47088C3.67523 6.22808 3.62343 5.96783 3.62343 5.705C3.62343 5.44217 3.67523 5.18192 3.77588 4.93912C3.87653 4.69632 4.02405 4.47575 4.21 4.29C4.39575 4.10405 4.61632 3.95653 4.85912 3.85588C5.10192 3.75523 5.36217 3.70343 5.625 3.70343C5.88783 3.70343 6.14808 3.75523 6.39088 3.85588C6.63368 3.95653 6.85425 4.10405 7.04 4.29L7.1 4.35C7.33568 4.58054 7.63502 4.73519 7.95941 4.794C8.28381 4.85282 8.61838 4.81312 8.92 4.68H9C9.29577 4.55324 9.54802 4.34276 9.72569 4.07447C9.90337 3.80618 9.99872 3.49179 10 3.17V3C10 2.46957 10.2107 1.96086 10.5858 1.58579C10.9609 1.21071 11.4696 1 12 1C12.5304 1 13.0391 1.21071 13.4142 1.58579C13.7893 1.96086 14 2.46957 14 3V3.09C14.0013 3.41179 14.0966 3.72618 14.2743 3.99447C14.452 4.26276 14.7042 4.47324 15 4.6C15.3016 4.73312 15.6362 4.77282 15.9606 4.714C16.285 4.65519 16.5843 4.50054 16.82 4.27L16.88 4.21C17.0657 4.02405 17.2863 3.87653 17.5291 3.77588C17.7719 3.67523 18.0322 3.62343 18.295 3.62343C18.5578 3.62343 18.8181 3.67523 19.0609 3.77588C19.3037 3.87653 19.5243 4.02405 19.71 4.21C19.896 4.39575 20.0435 4.61632 20.1441 4.85912C20.2448 5.10192 20.2966 5.36217 20.2966 5.625C20.2966 5.88783 20.2448 6.14808 20.1441 6.39088C20.0435 6.63368 19.896 6.85425 19.71 7.04L19.65 7.1C19.4195 7.33568 19.2648 7.63502 19.206 7.95941C19.1472 8.28381 19.1869 8.61838 19.32 8.92V9C19.4468 9.29577 19.6572 9.54802 19.9255 9.72569C20.1938 9.90337 20.5082 9.99872 20.83 10H21C21.5304 10 22.0391 10.2107 22.4142 10.5858C22.7893 10.9609 23 11.4696 23 12C23 12.5304 22.7893 13.0391 22.4142 13.4142C22.0391 13.7893 21.5304 14 21 14H20.91C20.5882 14.0013 20.2738 14.0966 20.0055 14.2743C19.7372 14.452 19.5268 14.7042 19.4 15Z" 
      stroke="currentColor" 
      strokeWidth="1" 
      strokeLinecap="round" 
      strokeLinejoin="round"
      opacity="0.7"
    />
    <circle cx="8" cy="8" r="1" fill="currentColor" opacity="0.6" />
    <circle cx="16" cy="16" r="1" fill="currentColor" opacity="0.6" />
  </svg>
);

export const BrandIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-8 h-8 ${className}`} viewBox="0 0 32 32" fill="none" {...props}>
    <defs>
      <linearGradient id="brandGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="oklch(0.60 0.20 320)" />
        <stop offset="50%" stopColor="oklch(0.55 0.25 310)" />
        <stop offset="100%" stopColor="oklch(0.65 0.18 330)" />
      </linearGradient>
      <filter id="glow">
        <feGaussianBlur stdDeviation="3" result="coloredBlur"/>
        <feMerge> 
          <feMergeNode in="coloredBlur"/>
          <feMergeNode in="SourceGraphic"/>
        </feMerge>
      </filter>
    </defs>
    <rect width="32" height="32" rx="12" fill="url(#brandGradient)" filter="url(#glow)" />
    <path d="M8 12L16 8L24 12L16 16L8 12Z" fill="white" opacity="0.95" />
    <path d="M8 16L16 12V20L8 24V16Z" fill="white" opacity="0.8" />
    <path d="M24 16L16 12V20L24 24V16Z" fill="white" opacity="0.6" />
    <circle cx="16" cy="12" r="2" fill="white" opacity="0.9" />
  </svg>
);

export const SearchIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-4 h-4 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <circle cx="11" cy="11" r="8" stroke="currentColor" strokeWidth="2" fill="none" />
    <path d="m21 21-4.35-4.35" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    <circle cx="11" cy="11" r="3" fill="currentColor" opacity="0.1" />
  </svg>
);

export const BellIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <path 
      d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9" 
      stroke="currentColor" 
      strokeWidth="2"
      fill="currentColor"
      fillOpacity="0.1"
    />
    <path 
      d="M13.73 21c-.42.44-.94.8-1.73.8-.79 0-1.31-.36-1.73-.8" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <circle cx="18" cy="6" r="3" fill="oklch(0.60 0.20 320)" />
  </svg>
);

export const UserIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-4 h-4 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <circle 
      cx="12" cy="7" r="4" 
      stroke="currentColor" 
      strokeWidth="2"
      fill="currentColor"
      fillOpacity="0.1"
    />
    <path 
      d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" 
      stroke="currentColor" 
      strokeWidth="2"
      fill="none"
    />
  </svg>
);

export const MenuIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <path d="M3 12H21" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
    <path d="M3 6H21" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
    <path d="M3 18H21" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
  </svg>
);

export const PlusIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-4 h-4 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <path d="M12 5V19" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
    <path d="M5 12H19" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
  </svg>
);

export const SunIcon: React.FC<IconProps> = ({ className = "", ...props }) => (
  <svg className={`w-5 h-5 ${className}`} viewBox="0 0 24 24" fill="none" {...props}>
    <defs>
      <linearGradient id="sunGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="currentColor" stopOpacity="1" />
        <stop offset="100%" stopColor="currentColor" stopOpacity="0.6" />
      </linearGradient>
    </defs>
    <circle 
      cx="12" 
      cy="12" 
      r="4" 
      fill="url(#sunGradient)" 
      stroke="currentColor" 
      strokeWidth="1.5"
      fillOpacity="0.2"
    />
    <line 
      x1="12" 
      y1="1" 
      x2="12" 
      y2="3" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <line 
      x1="12" 
      y1="21" 
      x2="12" 
      y2="23" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <line 
      x1="4.22" 
      y1="4.22" 
      x2="5.64" 
      y2="5.64" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <line 
      x1="18.36" 
      y1="18.36" 
      x2="19.78" 
      y2="19.78" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <line 
      x1="1" 
      y1="12" 
      x2="3" 
      y2="12" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <line 
      x1="21" 
      y1="12" 
      x2="23" 
      y2="12" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <line 
      x1="4.22" 
      y1="19.78" 
      x2="5.64" 
      y2="18.36" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
    <line 
      x1="18.36" 
      y1="5.64" 
      x2="19.78" 
      y2="4.22" 
      stroke="currentColor" 
      strokeWidth="2" 
      strokeLinecap="round"
    />
  </svg>
);
